/** Sprint 48 — Hospital ECG examination session types */

export const EXAMINATION_SESSION_MARKER = "__Sprint48ExaminationSession__";
export const EXAMINATION_PIPELINE_VERSION = "examination-workflow-v48.0";
...
export type ExaminationSessionRecord = {
  archivedAt?: string;
  caseId: string;
  ...
  patientId?: string;
  pipelineVersion: typeof EXAMINATION_PIPELINE_VERSION;
...
export function lifecycleForStep(stepId: ExaminationStepId): ExaminationLifecycleStatus {
  if (stepId === "acquire-ecg") return "acquiring";
  if (stepId === "digitize-ecg" || stepId === "quality-check" || stepId === "signal-review") return "digitizing";
  if (stepId === "measurement-studio" || stepId === "ai-cardiologist" || stepId === "clinical-decision-support") return "analyzing";
  if (
    stepId === "diagnostic-review" ||
    stepId === "final-impression" ||
    stepId === "doctor-validation" ||
    stepId === "electronic-signature" ||
    stepId === "generate-final-report"
  ) {
    return "review";
  }
  if (stepId === "archive-examination") return "archived";
  if (stepId === "create-examination") return "pending";
  return "pending";
}

export function nextExaminationStep(stepId: ExaminationStepId): ExaminationStepId | null {
  const index = EXAMINATION_STEP_ORDER.findIndex((row) => row.id === stepId);
  if (index < 0 || index >= EXAMINATION_STEP_ORDER.length - 1) return null;
  return EXAMINATION_STEP_ORDER[index + 1]!.id;
}
