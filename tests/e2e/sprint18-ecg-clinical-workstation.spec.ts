...
test.describe("Sprint 18 ECG Clinical Workstation 2.0 @sprint18", () => {
  ...
  test("workstation 2.0 shell and grouped toolbar visible", async ({ page }) => {
    ...
    await expect(page.getByText("ECG Pro Clinical Workstation 2.0")).toBeVisible();
    await expect(page.getByTestId("sprint18-toolbar-group-file")).toBeVisible();
    await expect(page.getByTestId("sprint18-toolbar-group-viewer")).toBeVisible();
    await expect(page.getByTestId("sprint18-toolbar-group-clinical")).toBeVisible();
    await expect(page.getByTestId("sprint18-view-mode-switcher")).toBeVisible();
  });
  ...
  test("view mode switching: monitor, waveform, compare", async ({ page }) => {
    ...
    await page.getByTestId("sprint18-view-mode-waveform").click();
    await expect(page.getByTestId("sprint18-waveform-view")).toBeVisible();
    await page.getByTestId("sprint18-view-mode-compare").click();
